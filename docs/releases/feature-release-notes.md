...
| 2026-08-25 | polish 帧合成器全 feature | Linux/Windows 默认 `--engine compositor`：idle 时间重映射 → zoom → lens warp → camera motion blur → cursor（depress + cursor motion blur）→ keystroke chips；`--engine ffmpeg` 保留旧滤镜；macOS 仍用 ffmpeg+ASS 并接受 `--engine` CLI 对齐。未 vendoring 专有源码。 | Linux/Windows 落地；macOS CLI 对齐 |
| 2026-08-25 | Computer Use 1–5 + 三端 e2e | 坐标 API↔display 缩放、click/scroll 修饰键、type 分批/换行→Return、key --hold-ms、mouse_down/up、事件完成后打点；polish SVG 热点/CursorType/keystroke 阴影；render-plan.json 导出/消费；`--quality anyos` + 120fps；`record proxy`（crf17/keyint=1）；Linux/Windows/macOS CLI 对齐；真实桌面录制+合成器验证。 | 三端 |
| 2026-08-25 | 三端显示级桌面命令 | Windows 与 macOS 补齐与 Linux 对齐的整屏 CLI：跨平台可用同一套 `screenshot` / `cursor-position` / `input` / `record` 观察、操控与录制整个桌面，脚本无需按平台分支。 | 发布 `1.1.0`：Windows 端 `screenshot`（GDI 虚拟桌面整屏）、`cursor-position`（GetCursorPos JSON）、`input`（复用 SendInput 层，`OPEN_COMPUTER_USE_WINDOWS_ALLOW_FOREGROUND_INPUT=1` 门控，Windows/Meta 键硬拒）、`record`（ffmpeg gdigrab + Ctrl+Break 优雅收尾，实验性）；macOS 端同名命令走 CGEvent / `CGDisplayCreateImage` 多屏合成 / `screencapture -v`，新增 `OPEN_COMPUTER_USE_MACOS_ALLOW_FOREGROUND_INPUT=1` 门控并复用 app agent 的 TCC 权限身份；三端命令名、参数与输出 JSON 结构对齐，均为 CLI-only，14 个 MCP tool 面不变；顺手修复 dependency-review `fail-on-scopes` 空值导致 supply-chain CI 常挂的问题。 |
...
| 2026-04-20 | macOS 分发签名 | `Open Computer Use` 的 release `.app` 已经能走统一的 `Developer ID Application` 签名链。 | 发布 `0.1.17`，接通 `Developer ID Application` 证书导入与统一签名链路。 |
