...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-08-25 | record 对齐 Cursor RecordScreen 画质能力 | 三端录屏默认画质、生命周期与脚本接口对齐：更高清晰度、可丢弃废片、可命名保存，macOS 有 ffmpeg 时也能出与 Linux/Windows 同款 mp4。 | 对照 Cursor `RecordScreen` 的采集参数（60fps 可选、veryfast/crf17/High/faststart、discard、save_as）升级三端 `record`：新增 `--quality demo\|draft`、`--draw-mouse`、`discard`、`stop --save-as`、启动就绪等待与更丰富 status；Linux/Windows ffmpeg 默认走 demo 画质；macOS 优先 ffmpeg avfoundation，回退 screencapture，并修复 start 忽略自定义 `--pidfile` 的问题。未移植 RecordScreen 专有的点击涟漪/缩放/空闲剪辑后处理管线。 |
