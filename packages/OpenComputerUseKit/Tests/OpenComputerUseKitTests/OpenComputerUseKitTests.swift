...
final class OpenComputerUseKitTests: XCTestCase {
    ...
    func testToolDefinitionCount() {
        XCTAssertEqual(ToolDefinitions.all.count, 14)
    }
    ...
    func testPreferredPermissionAppBundleURLPrefersInstalledCopyOverTransientRunningCopy() {
        ...
        let running = URL(fileURLWithPath: "/Users/example/projects/open-computer-use/dist/Open Computer Use.app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/open-computer-use-debug/dist/Open Computer Use.app")
    ...
    func testPreferredPermissionAppBundleURLPrefersRunningDevelopmentCopy() {
        ...
        let running = URL(fileURLWithPath: "/Users/example/projects/open-computer-use/dist/Open Computer Use (Dev).app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/open-computer-use-debug/dist/Open Computer Use (Dev).app")
    ...
    func testPreferredPermissionAppBundleURLCanPreferRunningReleaseCopyOverStaleInstalledCopy() {
        let staleInstalled = URL(fileURLWithPath: "/Users/example/projects/open-computer-use/dist/npm/open-computer-use/dist/Open Computer Use.app")
    ...
    func testToolDescriptionsMatchOfficialComputerUseSurface() {
        ...
        // Official window2 surface: five new tools must exist and share the
        // cross-platform definitions with the Windows runtime.
        for window2Tool in ["list_windows", "get_window", "launch_app", "get_window_state", "activate_window"] {
            XCTAssertNotNil(tools[window2Tool], "missing window2 tool \(window2Tool)")
        }
        let getWindowStateSchema = tools["get_window_state"]?.inputSchema
        XCTAssertEqual(getWindowStateSchema?["required"] as? [String], ["window"])
        let getWindowStateProperties = getWindowStateSchema?["properties"] as? [String: [String: Any]]
        XCTAssertNotNil(getWindowStateProperties?["include_screenshot"])
        XCTAssertNotNil(getWindowStateProperties?["include_text"])
        // Action tools accept the official window/screenshotId arguments.
        for actionTool in ["click", "drag", "press_key", "scroll", "set_value", "type_text", "perform_secondary_action"] {
            let properties = tools[actionTool]?.inputSchema["properties"] as? [String: [String: Any]]
            XCTAssertNotNil(properties?["window"], "\(actionTool) must accept a window argument")
        }
        XCTAssertEqual(
