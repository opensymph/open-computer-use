...
func TestWindow2ToolsReportNotSupportedYet(t *testing.T) {
	for _, name := range []string{"list_windows", "get_window", "launch_app", "get_window_state", "activate_window"} {
		if _, ok := findToolDefinition(t, name).InputSchema["properties"]; !ok {
			t.Fatalf("missing window2 tool definition %q", name)
		}
	}
	result := newService().callTool("list_windows", map[string]any{})
	if !result.IsError || !strings.Contains(result.Content[0].Text, "not supported yet on Linux") {
		t.Fatalf("list_windows result = %#v", result)
	}
	result = newService().callTool("click", map[string]any{"window": map[string]any{"id": 7}, "x": 1.0, "y": 2.0})
	if !result.IsError || !strings.Contains(result.Content[0].Text, "Window-targeted actions are not supported yet on Linux") {
		t.Fatalf("window-targeted click result = %#v", result)
	}
}
...
func TestLinuxRuntimeDocumentsATSPIAndFallbackBoundary(t *testing.T) {
	// The runtime is pure Go over AT-SPI2; coordinate input stays an explicit,
	// separately-wired fallback path on the runtime hooks.
	fr := newFakeRuntime(editorFixture())
	x, y := 5.0, 6.0
	resp := performOperation(fr.rt, &linuxRequest{
		Tool: "click", App: "Text Editor", ClickMethod: "global",
		MouseButton: "left", X: &x, Y: &y,
	})
	if !resp.OK || len(fr.mouseEvents) == 0 {
		t.Fatalf("coordinate click must reach the mouse hook: %v %q", fr.mouseEvents, resp.Error)
	}
	fr = newFakeRuntime(editorFixture())
	resp = performOperation(fr.rt, &linuxRequest{Tool: "press_key", App: "Text Editor", Key: "a"})
	if !resp.OK || len(fr.keyEvents) == 0 {
		t.Fatalf("press_key must reach the keyboard hook: %v %q", fr.keyEvents, resp.Error)
	}
...
func TestLinuxRuntimeTextLimitSupportsMaxMode(t *testing.T) {
	if atspiDefaultTextLimit != 500 {
		t.Fatal("Linux runtime should define the shared 500 character text limit")
	}
	if limit := parseTextLimit("max", atspiDefaultTextLimit); limit != nil {
		t.Fatal("Linux runtime should support max text limit mode")
	}
	if limit := parseTextLimit("MAX", atspiDefaultTextLimit); limit != nil {
		t.Fatal("max text limit mode is case-insensitive")
	}
	if limit := parseTextLimit(nil, atspiDefaultTextLimit); limit == nil || *limit != 500 {
		t.Fatal("absent text_limit should fall back to the 500 character default")
	}
	// Truncation reads one extra character so the ellipsis only appears when
	// the text actually exceeds the limit.
	if got := limitText("12345", limits(4)); got != "1234..." {
		t.Fatalf("limitText overflow = %q", got)
	}
	if got := limitText("1234", limits(4)); got != "1234" {
		t.Fatalf("limitText at limit = %q", got)
	}
}

func TestLinuxRuntimeTreeBudgetDefaultsMatchMacOS(t *testing.T) {
	if atspiMaxElements != 1200 {
	...
	// Request overrides flow through positive_int, exactly like runtime.py.
	if positiveInt(nil, atspiMaxElements) != 1200 || positiveInt(0, atspiMaxDepth) != 64 {
		t.Fatal("tree budgets must fall back to the shared defaults")
	}
}
