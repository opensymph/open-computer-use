...
func helpText(command string) string {
	...
	case "input":
		return "Usage:\n  open-computer-use input <action> [--display <:N>] [--api-size WxH] [options]\n\nActions (backed by xdotool; global synthetic input):\n  move <x> <y>\n  click [--button left|right|middle] [--count N] [--x X --y Y] [--modifiers ctrl+shift]\n  mouse_down|mouse_up [--button left] [--x X --y Y] [--modifiers ...]\n  drag <from_x> <from_y> <to_x> <to_y> [--button left]\n  scroll <up|down|left|right> [--amount N] [--x X --y Y] [--modifiers ...]\n  type <text>                 newlines become Return; long text is batched\n  key <key-or-chord> [--hold-ms N]\n  wait <seconds>\n\n`--api-size` (or OPEN_COMPUTER_USE_API_SIZE) scales coords from API canvas\nto the display. Events are logged after each action completes.\nRequires OPEN_COMPUTER_USE_ALLOW_GLOBAL_POINTER_FALLBACKS=1.\n"
	case "record":
		return "Usage:\n  open-computer-use record start [--display <:N>] [--output <path.mp4>] [--fps N]\n                         [--quality demo|draft|proxy|anyos] [--draw-mouse 0|1] [--polish] [--pidfile <path>]\n  open-computer-use record stop  [--pidfile <path>] [--save-as <name-or-path>] [--polish]\n  open-computer-use record discard [--pidfile <path>]\n  open-computer-use record polish --input <raw.mp4> [--events <file>] [--output <polished.mp4>]\n                         [--engine compositor|ffmpeg] [--plan <render-plan.json>]\n                         [--write-plan <path>] [--no-write-plan]\n                         [--cursor-style slow|mellow|quick|rapid]\n                         [--ripples] [--no-ripples] [--no-keystrokes] [--no-cursor]\n                         [--no-idle-speedup] [--no-zoom]\n  open-computer-use record proxy --input <raw.mp4> [--output-dir <dir>] [--no-1080p] [--no-full]\n  open-computer-use record status [--pidfile <path>]\n\nRecord the X11 display with ffmpeg x11grab. While recording, display `input`\nactions append <output>.events.json after each action completes. `record polish`\ndefaults to a clean-room frame compositor (idle remap → zoom → lens warp →\nmotion blur → yellow click ripples → cursor + keystrokes;\nripples on by default, `--no-ripples` to disable) and writes <stem>.render-plan.json.\n`--quality anyos` is all-intra (prefer --fps 120). `record proxy` builds\nseekable 1080p/full proxies (crf17/veryfast/keyint=1).\n"
	default:
