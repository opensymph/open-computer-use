...
func helpText(command string) string {
	...
	case "record":
		return "Usage:\n  open-computer-use.exe record start [--output <path.mp4>] [--fps N]\n                              [--quality demo|draft|proxy] [--draw-mouse 0|1] [--pidfile <path>]\n  open-computer-use.exe record stop  [--pidfile <path>] [--save-as <name-or-path>]\n  open-computer-use.exe record discard [--pidfile <path>]\n  open-computer-use.exe record status [--pidfile <path>]\n\nRecord the desktop with ffmpeg gdigrab (H.264 mp4); ffmpeg must be on PATH.\nstart runs ffmpeg detached; stop raises Ctrl+Break so the mp4 is finalized;\ndiscard stops and deletes the output (Cursor RecordScreen DISCARD parity).\nDefaults: fps 30, quality demo (RecordScreen-aligned veryfast/crf17/High/\nfaststart), draw-mouse 1, output in %TEMP%, pidfile\n%TEMP%\\open-computer-use-record.pid. Use --quality draft for ultrafast, or\n--fps 60 for RecordScreen-like capture rate.\n"
	default:
...
Commands:
  ...
  record <start|stop|discard|status>  Record the desktop with ffmpeg gdigrab.
